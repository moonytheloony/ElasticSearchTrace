namespace TraceService
{
    ...
    public class ConcurrentEventSender<EventDataType> : IDisposable
    {
        ...
        private readonly TimeSpanThrottle eventLossThrottle;

        private readonly BlockingCollection<EventDataType> events;

        private readonly IHealthReporter healthReporter;

        private readonly uint maxConcurrency;

        private readonly TimeSpan noEventsDelay;

        private readonly Func<IEnumerable<EventDataType>, long, CancellationToken, Task> TransmitterProc;
        ...
        public bool ApproachingBufferCapacity
        {
            get
            {
                return this.events.Count >= this.capacityWarningThreshold;
            }
        }
        ...
        public void SubmitEvent(EventDataType eData)
        {
            if (!this.events.TryAdd(eData))
            {
                ...
                this.eventLossThrottle.Execute(
                    () =>
                        {
                            this.healthReporter.ReportProblem(
                                "Diagnostic events buffer overflow occurred. Some diagnostics data was lost.");
                        });
            }
        }
        ...
        {
            var transmitterTasks = new List<Task>((int)this.maxConcurrency);
        ...
        private bool GetBatch(
            CancellationToken cancellationToken,
            out IEnumerable<EventDataType> eventsToSend,
            out int eventsFetched)
        {
            ...
            IHealthReporter healthReporter)
        {
            if (eventBufferSize <= 0)
            {
                throw new ArgumentOutOfRangeException(
                    "eventBufferSize",
                    "Event buffer size should be greater than zero");
            }
